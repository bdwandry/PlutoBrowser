...
static void keyboardGetButtonState(PDButtons *current, PDButtons *pushed, PDButtons *released) {
...
static void enterNewLetterIfNecessary(PDKeyboard * _Nonnull self) {
    ...
    keyboardGetButtonState(&pressing, &justPressed, NULL);
...
static void checkButtonInputs(PDKeyboard * _Nonnull self) {
    ...
    keyboardGetButtonState(&pressing, &justPressed, &justReleased);
...
static PDKeyboard * _Nonnull PDKeyboardNew(void) {
    ...
    loadFontAndImages();
    logger_log("KB new: font+images loaded");
