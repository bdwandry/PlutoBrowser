...
{
    int rows = (count + 1) / 2;
    int bottom = 24 + 4 + 172 + rows * 54 + 80 + 8; /* home_content_bottom */
    int max = bottom - 240;
    return max < 0 ? 0 : max;
}
...
int main(int argc, char **argv)
{
    ...
    int count = g_bmCount;
    printf("== BF14 home crank harness (count=%d) ==\n", count);
    ...
    /* TC3: crank UP from bottom returns exactly to Settings (0). */
    crank((float)-(count + 4) * HOME_CRANK_STEP_PX); /* overshoot past the top */
